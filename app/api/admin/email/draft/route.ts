...
Write the BODY of a professional email — warm, elegant, refined British English.
STRICT LENGTH: at most 2 short paragraphs, no more than ~60 words each (120 words total, hard limit). Never enumerate or list every property, rate, offer or detail the message mentions — a brief, general acknowledgement referencing at most one or two specifics is enough. This applies even to long marketing/partner emails listing many properties or rates — do not try to address each one.
Do NOT include a greeting line (no "Dear ...") and NO sign-off — those are added automatically by our template.
When replying to a message, acknowledge it and answer any direct questions helpfully, without restating everything it contains.
...
Context / notes: ${String(context || "none").slice(0, 800)}`;
  ...
  if (aiConfigured()) {
    const { text } = await generateText(SYSTEM, [{ role: "user", content: prompt }], 1200);
