...
export default function MiniAreaChart({ data }: { data: { label: string; count: number }[] }) {
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" role="img" aria-label="Bookings over the last 14 days">
        ...
        {ticks.map(t => (
          <g key={t}>
            ...
            <text x={padL - 6} y={Y(t) + 3} textAnchor="end" fontSize="9" fill="currentColor" fillOpacity="0.45">{t}</text>
          </g>
        ))}
        ...
        {data.map((d, i) => (
          <g key={i}>
            <circle cx={X(i)} cy={Y(d.count)} r="3" fill="#FFD400" stroke="currentColor" strokeWidth="1.5" className="chart-dot" style={{ animationDelay: `${0.5 + i * 0.05}s` }} />
