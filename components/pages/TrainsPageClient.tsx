...
function TrainsContent({ trains }: { trains: Train[] }) {
  const { t } = useLanguage();
  ...
  const filtered = useMemo(() => trains
    ...
    [trains, fromCode, toCode, classes, times]
  );
...
export default function TrainsPageClient({ trains }: { trains: Train[] }) {
  return (
    <Suspense>
      <TrainsContent trains={trains} />
    </Suspense>
  );
}
