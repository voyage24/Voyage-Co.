...
export default function BlogPageClient({ posts }: { posts: BlogPost[] }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 pb-16">
      ...
      {/* Featured */}
      {posts[0] && (
        <Link href={`/blog/${posts[0].slug}`} className="block bg-vc-800 rounded-2xl p-6 sm:p-10 mb-10 border border-vc-700 hover:border-[#b09e74]/50 transition-colors group">
          ...
          <h2 className="font-serif text-2xl sm:text-3xl font-light text-[#ece7dd] mt-4 mb-2 group-hover:text-[#b09e74] transition-colors">{posts[0].title}</h2>
          <p className="text-[#9aa4ab] text-sm mb-4 font-light max-w-2xl leading-relaxed">{posts[0].excerpt}</p>
