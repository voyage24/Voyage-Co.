...
export default function OutstationCabCard({ cab }: { cab: OutstationCab }) {
    ...
    <div className="bg-panel rounded-2xl border border-line hover:border-gold/40 shadow-card hover:shadow-card-hover hover:-translate-y-1.5 transition-all duration-500 overflow-hidden group">
      <div className="relative aspect-[16/9] overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex gap-2">
          ...
          <span className="text-[9px] font-medium tracking-[0.15em] uppercase text-white/90 bg-vc-950/50 backdrop-blur-sm px-2.5 py-1 rounded-sm">{cab.tripType}</span>
        </div>
      ...
      <div className="p-5">
        <p className="text-[11px] tracking-[0.14em] uppercase text-gold mb-1 font-light">{cab.vehicleType}</p>
        <Link href={`/outstation-cabs/${cab.id}`}>
        ...
        {cab.durationEstimate && (
          <div className="flex items-center gap-3 text-xs text-ink-muted mb-4 font-light">
            <span className="flex items-center gap-1"><Clock size={11} className="text-gold" /> {cab.durationEstimate}</span>
          </div>
        )}
        ...
        <div className="flex items-end justify-between pt-4 border-t border-line">
          <div>
            <p className="text-[10px] tracking-[0.1em] uppercase text-ink-faint font-light">{cab.priceOnRequest ? "Price" : "From"}</p>
            <p className="font-serif text-2xl font-light text-ink">{cab.priceOnRequest ? "On request" : format(cab.price)}</p>
          </div>
          <Link href={`/book?type=outstation-cab&id=${cab.id}`} className="px-5 py-2.5 border border-line-strong text-ink hover:bg-ink hover:text-page text-xs font-normal tracking-[0.12em] uppercase rounded-sm transition-all duration-300">
            Reserve
          </Link>
        </div>
      </div>
