...
import { Plane, BedDouble, Utensils, ShoppingBag, Ticket, Car, Receipt, Plus, Loader2, Trash2 } from "lucide-react";
...
type Expense = { id: string; description: string; amount: number; category: string; spentOn: string; bookingRef: string | null };
...
export default function PersonalExpenses({ initialRef }: { initialRef: string }) {
  ...
  const add = async () => {
    ...
    setBusy(true);
    await fetch("/api/account/expenses", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ description: desc, amount: Number(amount), category, spentOn, bookingRef: tripRef || null }) });
          ...
          return (
            <div key={e.id} className="flex items-center gap-3 rounded-xl border border-line bg-panel px-4 py-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[11px] text-ink-faint">{e.category} · {e.spentOn}</p>
              </div>
