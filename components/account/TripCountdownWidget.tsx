...
export default function TripCountdownWidget({ checkIn, checkOut, title }: { checkIn: string | null; checkOut: string | null; title: string }) {
  ...
  return (
    <div className="rounded-2xl border border-gold/30 bg-panel p-6 mb-4 shadow-card">
      ...
      <div className="flex items-end gap-3">
        <span className="font-serif text-5xl sm:text-6xl font-light leading-none text-ink">{big}</span>
        <span className="text-sm text-ink-muted pb-1.5">{small}</span>
      </div>
      <p className="text-sm text-ink-muted font-light mt-2 truncate">{title}</p>
      {phase === "during" && (
